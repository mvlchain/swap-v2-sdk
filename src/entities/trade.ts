...
import { _1000, _997, ONE, ZERO } from '../constants';
...
> {
  ...
  public constructor(
    ...
    if (tradeType === TradeType.EXACT_INPUT) {
      ...
      for (let i = 0; i < route.path.length - 1; i++) {
        ...
        const [outputAmount] = pair.getOutputAmount(tokenAmounts[i], feeValue);
      ...
      for (let i = route.path.length - 1; i > 0; i--) {
        ...
        const [inputAmount] = pair.getInputAmount(tokenAmounts[i], feeValue);
   ...
   * @param feeValue used for custom fee values; defaults to 997 means 100 - 99.7 = 0.3%
   */
  public static bestTradeExactIn<TInput extends Currency, TOutput extends Currency>(
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      try {
        [amountOut] = pair.getOutputAmount(amountIn, feeValue);
      } catch (error) {
        // input too low
        // @ts-ignore
